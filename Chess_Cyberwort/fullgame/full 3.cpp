...
void kill(int a,int b, int c, int d, char q);
...
// INITIALIZATION OF BLOCKS
    ...
    draw(selector_rect.x, selector_rect.y, killer);
    // DRAW THE SELECTOR
...
void kill(int a, int b, int c, int d, char q)
{
    if(c=='y')
    {
        cout<<"selector pos: "<<a<<" "<<b<<endl;
        cout<<"block pos: "<<c<<" "<<d<<endl;
        if(a==c&&b==d)
            eRect(a,b);
    }
}
